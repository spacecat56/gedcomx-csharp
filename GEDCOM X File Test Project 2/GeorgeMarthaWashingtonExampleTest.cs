...
namespace GEDCOM_X_File_Test_Project_2
{
    [TestClass]
    public class GeorgeMarthaWashingtonExampleTest
    {
        private DefaultXmlSerialization xmlSerializer = new DefaultXmlSerialization();
        private DefaultJsonSerialization jsonSerializer = new DefaultJsonSerialization();
        ...
        public void TestExample()
        {
            ...
            Gx.Gedcomx xAndBack = xmlSerializer.Deserialize<Gx.Gedcomx>(xmlSerializer.Serialize(gx));
            Gx.Gedcomx jAndBack = jsonSerializer.Deserialize<Gx.Gedcomx>(jsonSerializer.Serialize(gx));

            Assert.IsNotNull(xAndBack);
            Assert.IsNotNull(jAndBack);

            Assert.AreEqual(george.Names[0].NameForm.FullText, xAndBack.Persons[0].Names[0].NameForm.FullText);
            Assert.AreEqual(martha.Names[0].NameForm.FullText, jAndBack.Persons[1].Names[0].NameForm.FullText);
        }
